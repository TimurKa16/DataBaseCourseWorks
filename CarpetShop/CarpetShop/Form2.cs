    ...
    public partial class Form2 : Form
    {
        ...
        private int but3()
        {
            ...
            try
            {
                ...
                sqlReader = null;
                command = new SqlCommand("SELECT [P_Sell] FROM [Product] WHERE [P_ID] = @P_ID", sqlConnection);
                command.Parameters.AddWithValue("P_ID", ID);
                sqlReader = command.ExecuteReader();

                while (sqlReader.Read())
                {
                    Cost = Convert.ToDouble(sqlReader["P_Sell"]);
                }
                if (sqlReader != null)
                    sqlReader.Close();
                Cost *= Number;
                ...
                command.Parameters.AddWithValue("P_ID", ID);
                command.Parameters.AddWithValue("P_Count", (Count - Number));
                ...
                command.Parameters.AddWithValue("G_Income", Income + Cost);
                command.Parameters.AddWithValue("G_Outgo", Outgo);
